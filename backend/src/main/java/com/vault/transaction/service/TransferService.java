...
public class TransferService {
    ...
    public TransactionResponse transfer(User user, TransferRequest request, String idempotencyKey) {
        ...
        try {
            ...
            Transaction transaction = executeTransferTx(fromAccount, request, idempotencyKey);
    ...
    public Transaction executeTransferTx(Account unLockedFromAcc, TransferRequest request, String idempotencyKey) {
        ...
        if (request.isInterBank()) {
            ...
            if (firstId.compareTo(secondId) < 0) {
                fromAccount = accountRepository.findByIdForUpdate(firstId).get();
                toAccount = accountRepository.findByIdForUpdate(secondId).get();
            } else {
                toAccount = accountRepository.findByIdForUpdate(secondId).get();
                fromAccount = accountRepository.findByIdForUpdate(firstId).get();
            }
